import { defaults, filter, values, forEach } from 'lodash-es'
import Vue from 'vue'
/**
	...
	return {
      ...
      forEach(this.state.links, (link) => {
        if(link.points.length === 0){
          link.points.push(this.getPortCenter(this.getNode(link.source),link.sourcePort));
          link.points.push(this.getPortCenter(this.getNode(link.target),link.targetPort));
        }
      })
		},
      ...
      this.state.validators.onEdgeRemove().then(valid => {
        if (valid) {
